...
import {
  createCodegenImportSourceMap,
  SCALAR_TYPES_NAMESPACE,
} from "./imports";
...
const PACKAGE_NAME = "@ptdgrp/typedgql";
...
export class Generator {
  ...
  private async initializePrettier(): Promise<void> {
    ...
    if (this.options.prettier === true || this.isCustomOutputDir) {
      ...
      if (this.usePrettier) {
        console.log(
          "[typedgql] Prettier detected, will format generated files",
        );
      }
    }
  }
  ...
  private createCodeStream(path: string): WriteStreamLike | WriteStream {
    const writeBanner = (stream: WriteStreamLike | WriteStream) => {
  ...
  private get rootRuntimeImportSource(): string {
    return this.usesPackageInternalRuntimeImports
      ? "../dist/index.mjs"
      : PACKAGE_NAME;
  }

  private get nestedRuntimeImportSource(): string {
    return this.usesPackageInternalRuntimeImports
      ? "../../dist/index.mjs"
      : PACKAGE_NAME;
  }

  private get usesPackageInternalRuntimeImports(): boolean {
    return normalize(this.targetDir) === normalize(DEFAULT_TARGET_DIR);
  }
  ...
  private async flushBufferedStreams(): Promise<void> {
    ...
    console.log(
      `[typedgql] Formatting ${this.bufferedStreams.length} files with prettier...`,
    );
  ...
  private async generateInputTypes(inputTypes: GraphQLInputObjectType[]) {
    ...
    const promises = inputTypes.map(async (type) => {
      const stream = this.createCodeStream(
        join(dir, `${toKebabCase(type.name)}.ts`),
      ) as WriteStream;
  ...
  private async generateTypeHierarchy(
    ...
    const stream = this.createCodeStream(
      join(this.targetDir, "type-hierarchy.ts"),
    ) as WriteStream;
  ...
  private async generateEnumInputMetadata(schema: GraphQLSchema) {
    const stream = this.createCodeStream(
      join(this.targetDir, "enum-input-metadata.ts"),
    ) as WriteStream;
    new EnumInputMetadataWriter(
      schema,
      stream,
      this.options,
      this.generatedImportSourceMap,
  ...
  private async generateAsyncRuntime() {
    const stream = this.createCodeStream(
      join(this.targetDir, "client-runtime.ts"),
    ) as WriteStream;
    stream.write(
      ASYNC_CODE.replaceAll("../dist/index.mjs", this.rootRuntimeImportSource),
    );
    await endStream(stream);
  }
  ...
  private async generateScalarTypes() {
    const stream = this.createCodeStream(
      join(this.targetDir, "scalar-types.ts"),
    ) as WriteStream;
  ...
  private async writeIndex(schema: GraphQLSchema, ctx: SelectionContext) {
    const stream = this.createCodeStream(
      join(this.targetDir, "index.ts"),
    ) as WriteStream;
    const selectionSuffix = this.options.selectionSuffix ?? "Selection";
    stream.write(
      `import type { Selection, ExecutableSelection, SchemaType, ShapeOf, VariablesOf } from "${this.rootRuntimeImportSource}";\n`,
    );
    stream.write(
      `import { FragmentRef, createSelection, resolveRegisteredSchemaType } from "${this.rootRuntimeImportSource}";\n`,
    );
    for (const type of ctx.selectionTypes) {
