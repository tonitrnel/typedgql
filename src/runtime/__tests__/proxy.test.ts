...
describe("关联字段回调选择", () => {
  it("callback parameter can chain child scalar fields from registered schema", () => {
    ...
    const postSelection = createSelection<
      string,
      Selection<string, object, object>
    >(postType, enumInputMetadata, undefined);

    const result = (postSelection as any).comments(
      (comment: any) => comment.id.body,
    );
  ...
  it("association field auto-parameterizes required args when omitted", () => {
    ...
    const selection = createSelection<
      string,
      Selection<string, object, object>
    >(queryType, enumInputMetadata, undefined);
  ...
  it("association field does not auto-parameterize optional args", () => {
    ...
    expect(postsField!.args).toBeDefined();
  });
  ...
  it("$use(fragmentRef) embeds named fragment", () => {
    ...
    const fragment = new FragmentRef(
      "chatFields",
      (childSelection as any).id.role,
    );
  ...
  it("$directive() applies to selection when there is no preceding field", () => {
    const selection = createSelection<
      string,
      Selection<string, object, object>
    >(makeType("Root", ["id", "name"]), enumInputMetadata, undefined);
  ...
  it("$directive() applies to last field and $include/$skip are available", () => {
    ...
    const withFieldDirective = (selection as any).id.$directive("deprecated", {
      reason: "x",
    });
  ...
  it("field-level directives are appended by name and preserve existing args/child", () => {
    ...
    expect(
      userField?.fieldOptionsValue?.directives.get("cacheControl"),
    ).toEqual({ maxAge: 60 });
    expect(userField?.fieldOptionsValue?.directives.get("auth")).toEqual({
      role: "ADMIN",
    });
  });
  ...
  it("$include/$skip apply to selection when there is no preceding field", () => {
    const selection = createSelection<
      string,
      Selection<string, object, object>
    >(makeType("RootIncludeSkip", ["id"]), enumInputMetadata, undefined);
  ...
  it("$alias throws without a preceding field", () => {
    const selection = createSelection<
      string,
      Selection<string, object, object>
    >(makeType("RootAliasError", ["id"]), enumInputMetadata, undefined);
  ...
  it("scalar fields with args use method handler and support options callback", () => {
    ...
    expect(withAutoArgs.toString()).toContain("search(q: $q, limit: $limit)");

    const withOptions = (selection as any).search(
      { q: "abc", limit: 1 },
      (opts: any) =>
        opts.alias("quickSearch").directive("include", { if: true }),
    );
    const field = withOptions.fieldMap.get("quickSearch");
    expect(field?.name).toBe("search");
    expect(field?.fieldOptionsValue?.directives.get("include")).toEqual({
      if: true,
    });
  });
  ...
  it("association fields fail fast when target type is invalid", () => {
    ...
    const missingTargetSelection = createSelection<
      string,
      Selection<string, object, object>
    >(missingTargetType, enumInputMetadata, undefined);
    expect(() =>
      (missingTargetSelection as any).badRef((x: any) => x.id),
    ).toThrow('Field "badRef" has no target type');
    ...
    const unresolvedTargetSelection = createSelection<
      string,
      Selection<string, object, object>
    >(unresolvedTargetType, enumInputMetadata, undefined);
    expect(() =>
      (unresolvedTargetSelection as any).badRef((x: any) => x.id),
    ).toThrow(
  ...
  it("proxy exposes schemaType via property access", () => {
    ...
    const selection = createSelection<
      string,
      Selection<string, object, object>
    >(rootType, enumInputMetadata, undefined);
  ...
  it("$directive after omitting the last field still rewrites field options", () => {
    const selection = createSelection<
      string,
      Selection<string, object, object>
    >(makeType("RootDirectiveAfterOmit", ["id"]), enumInputMetadata, undefined);

    const result = (selection as any).id
      .$omit("id")
      .$directive("include", { if: true });
    const idField = result.fieldMap.get("id");
    expect(idField?.name).toBe("id");
    expect(idField?.fieldOptionsValue?.directives.get("include")).toEqual({
      if: true,
    });
  });

  it("inherited function field auto-propagates args from merged fields map", () => {
    ...
    expect(result.toString()).toContain("q: $q");
  });
  ...
  it("$on with same-type builder does not force __typename", () => {
    ...
    const parentSelection = createSelection<
      string,
      Selection<string, object, object>
    >(nodeType, enumInputMetadata, undefined);
